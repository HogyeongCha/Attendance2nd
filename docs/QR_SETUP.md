...
- `db/seeds/attendance_days.seed.sql`
- `ops/qr/*.png`
- `ops/qr/*.svg`
- `ops/qr/manifest.json`

## 3. 운영 순서

1. `NEXT_PUBLIC_SITE_URL`를 실제 배포 URL로 맞춘다
2. `npm run generate:qr` 실행
3. 생성된 `db/seeds/attendance_days.seed.sql`을 Supabase SQL Editor 또는 migration으로 적용
4. `ops/qr/*.png` 중 해당 날짜 파일을 인쇄해 공도2 입구에 비치
...
- 현재 시간이 해당 날짜에 설정된 `start_time ~ end_time` 범위인지
- 사용자 세션이 유효한지
- GPS가 허용 반경 이내인지
