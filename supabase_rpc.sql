...
begin
  ...
  if not found then
    raise exception '등록된 참가자를 찾을 수 없습니다.';
  end if;

  if coalesce(nullif(trim(p_name), ''), '') <> '' and v_participant.name = trim(p_name) then
    return query
    select v_participant.id, v_participant.student_id, v_participant.name;
    return;
  end if;

  if coalesce(nullif(trim(p_phone_last4), ''), '') <> ''
     and right(regexp_replace(coalesce(v_participant.phone, ''), '\D', '', 'g'), 4) = trim(p_phone_last4) then
    return query
    select v_participant.id, v_participant.student_id, v_participant.name;
    return;
  end if;

  raise exception '이름 또는 전화번호 뒤 4자리가 일치하지 않습니다.';
end;
$$;
...
begin
  ...
  from public.attendance_days
  where day_number < v_day.day_number;
